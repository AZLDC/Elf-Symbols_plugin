...
## 商標與名稱聲明
本專案的 Logo 為作者自行設計，若發現與現有商標或名稱相似，請於 [GitHub Issues](../../issues) 留言通知，作者將配合進行修改或替換。(2026/03/05)
...
- 托盤切換：滑鼠左鍵點擊托盤圖示即可切換
- 托盤選單：滑鼠右鍵開啟選單，可開啟設定或結束程式
- 設定介面：可設定「登入後啟動」與「啟動時顯示 Logo」
- 狀態顯示：托盤提示文字顯示目前輸出模式與快捷說明
- 自動偵測登錄路徑：嘗試尋找可用的新注音（IME）登錄路徑並確保必要鍵值存在
- 立即生效：寫入後會切換到英文輸入法再切回新注音，促使新狀態立即套用
- 登入後自動啟動：預設關閉，於設定介面啟動。
- 防止重複啟動：透過系統具名 Mutex 確保同一時間只會執行一個實例，避免誤開多次。
...
- 已安裝微軟「新注音／注音」輸入法
- Python 3.8 以上
- 依賴套件：`keyboard`、`Pillow`（選用，用於顯示啟動 Logo）
...
### config.cfg 語系對應表
程式首次啟動時會在 `config.cfg` 中自動產生 `lang_labels` 區段，內含多種語系鍵盤的對應名稱（例如日文鍵盤、韓文鍵盤等）。這些對應會反映在托盤提示文字中，讓使用者切換到非注音輸入法時也能看到目前使用的語系名稱。

若切換到尚未收錄的語系，托盤提示文字會顯示類似「其他語系 (0x00000411)」的字樣，括號內即為該語系的 HKL 碼。若需新增或修改語系名稱，可直接編輯 `config.cfg` 中的 `lang_labels`，將 HKL 碼去掉 `0x` 前綴後作為鍵值填入即可。例如托盤顯示 `0x00000411`，則填入：
```json
"00000411": "日文鍵盤"
```
...
```bash
py -3 -m pip install keyboard Pillow
```
...
- 托盤：滑鼠左鍵點擊托盤圖示即可切換
- 設定：滑鼠右鍵開啟選單，點選「設定」可開啟設定視窗
- 退出：滑鼠右鍵開啟選單，點選「關閉APP」可關閉程式
...
## Third-Party Notices
本專案使用以下第三方套件，各套件授權以其原始授權為準：
- `keyboard`
- `Pillow`（選用，用於顯示啟動 Logo）