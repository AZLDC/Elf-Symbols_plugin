...
若切換到尚未收錄的語系，托盤提示文字會顯示類似「其他語系 (0x00070404)」的字樣，括號內即為該語系的 HKL 碼。若需新增或修改語系名稱，可直接編輯 `config.cfg` 中的 `lang_labels`，將 HKL 碼去掉 `0x` 前綴後作為鍵值填入即可。例如托盤顯示 `0x00070404`，則填入：
```json
"00070404": "天使文字鍵盤"
```
