...
1. 在檔案總管找到任一個 `.pyw`（例如 `新注音繁簡切換.pyw`）
...
1. 將 `run.bat` 與 `新注音繁簡切換.pyw` 放在同一資料夾
2. 點兩下 `run.bat`
...
- 托盤圖示同步顯示：
  ...
  - `簡.png`：簡體輸出
- 自動偵測登錄路徑：
  - 程式會嘗試尋找可用的新注音（IME）登錄路徑，並確保必要鍵值存在
- 立即套用：
  - 程式會對目前視窗送出語系重載請求，讓輸出模式盡快生效
...
1. 下載或複製本資料夾內容：
   ...
   - `繁.png`、`簡.png`、`轉.png`
   - `run.bat`（選用）
2. 執行 `新注音繁簡切換.pyw`（或執行 `run.bat`）
3. 若缺少依賴套件，程式會提示安裝方式
   - 若不允許自動安裝，請先在目標 Python 環境中完成安裝後再啟動
